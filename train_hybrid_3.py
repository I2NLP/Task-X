...
3-CLASS VERSION: Yes, No, Can't tell

Usage:
    python train_hybrid_3class.py
"""
...
# ============================================
# KEY CHANGE: 3-class label mapping
# ============================================
LABEL_TO_ID = {"No": 0, "Yes": 1, "Can't tell": 2}
ID_TO_LABEL = {0: "No", 1: "Yes", 2: "Can't tell"}
NUM_CLASSES = 3


class ConspiracyDataset(Dataset):
    """Dataset for conspiracy detection (3-class)."""
    ...
    def __getitem__(self, idx):
        ...
        if conspiracy_label in LABEL_TO_ID:
            label = LABEL_TO_ID[conspiracy_label]
        else:
...
class BertBiLSTMAttention(nn.Module):
    """
    Hybrid model: BERT + BiLSTM + Multi-Head Attention (3-class version)
    """
    ...
    def __init__(
        ...
        num_classes=NUM_CLASSES,  # Changed to 3
        dropout=0.3,
        freeze_bert=False
    ):
        super().__init__()
...
def load_data(file_path):
    """Load JSONL data - NO exclusions, keep all 3 classes."""
    data = []
    with open(file_path, 'r') as f:
        for line in f:
            ...
            # Keep all 3 classes: Yes, No, Can't tell
            if conspiracy in LABEL_TO_ID:
                data.append(item)
    return data
...
def main():
    ...
    CONFIG = {
        ...
        'output_dir': 'bert-lstm-attention-3class',
    }
    ...
    dist = Counter(d['conspiracy'] for d in train_data)
    ...
    # Class weights for 3 classes
    train_labels = [item['conspiracy'] for item in train_data]
    class_counts = {label: train_labels.count(label) for label in LABEL_TO_ID.keys()}
    total = len(train_labels)
    
    # Weight = total / (num_classes * count) - higher weight for minority class
    class_weights = torch.tensor([
        total / (NUM_CLASSES * class_counts['No']),
        total / (NUM_CLASSES * class_counts['Yes']),
        total / (NUM_CLASSES * class_counts["Can't tell"])
